...
import { scrapeTripoliBarNews } from '@/app/lib/scrapers/tripoli-bar'
...
export async function syncTripoliBarNews() {
  console.log('Starting Tripoli Bar news sync...')

  const syndicateId = await findSyndicateId(['tripoli', 'north', 'nl-bar'])
  if (!syndicateId) {
    console.error('Tripoli Bar syndicate not found')
    return { inserted: 0, found: 0 }
  }

  const sourceId = await resolveSourceId(syndicateId)
  if (!sourceId) {
    console.error('No source_id found for Tripoli Bar')
    return { inserted: 0, found: 0 }
  }
  ...
  try {
    const items = await scrapeTripoliBarNews()
