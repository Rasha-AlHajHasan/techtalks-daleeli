...
import { scrapePhysiciansNews } from '@/app/lib/scrapers/physicians'
...
export async function syncPhysiciansNews() {
  console.log('Starting Physicians news sync...')

  const syndicateId = await findSyndicateId(['physician', 'physiotherapist', 'lopt'])
  if (!syndicateId) {
    console.error('Physicians syndicate not found')
    return { inserted: 0, found: 0 }
  }

  const sourceId = await resolveSourceId(syndicateId)
  if (!sourceId) {
    console.error('No source_id found for Physicians')
    return { inserted: 0, found: 0 }
  }
  ...
  try {
    const items = await scrapePhysiciansNews()
